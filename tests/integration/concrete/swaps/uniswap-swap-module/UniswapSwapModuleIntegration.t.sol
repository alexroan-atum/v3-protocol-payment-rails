...
/// @title UniswapSwapModuleIntegrationTest
/// @notice Integration tests covering real PaymentRails + UniswapSwapModule interaction:
...
contract UniswapSwapModuleIntegrationTest is Test {
    /*//////////////////////////////////////////////////////////////////////////
                                EVENTS
    //////////////////////////////////////////////////////////////////////////*/
    ...
    function test_FullLifecycle_ActionExecuted_EmitsCorrectAmountOut() public {
        ...
        emit ActionExecuted(address(sellToken), "UNISWAP_SWAP", SELL_AMOUNT, BUY_AMOUNT, address(buyToken), keeper);
    ...
    function test_ValidationFailure_TokenNotEnabled_Reverts() public {
        ...
        vm.prank(owner);
        paymentRails.configureToken(address(sellToken), "UNISWAP_SWAP", address(module), MIN_BALANCE, params, false);
    ...
    function test_ModuleValidationFailure_SameTokens_ReturnsFalse() public {
        ...
        vm.prank(owner);
        paymentRails.configureToken(address(sellToken), "UNISWAP_SWAP", address(module), MIN_BALANCE, params, true);
    ...
    /// @dev Swap modules: reconfigure to a different UniswapSwapModule.
    function test_Reconfiguration_SwapToNewModule() public {
        ...
        UniswapSwapModule newModule = new UniswapSwapModule(owner);
        MockRouter newRouter = new MockRouter();
        buyToken.mint(address(newRouter), BUY_AMOUNT * 10);
        ...
        bytes memory params = abi.encode(address(buyToken));
        vm.prank(owner);
        paymentRails.configureToken(address(sellToken), "UNISWAP_SWAP", address(newModule), MIN_BALANCE, params, true);
    ...
    function _configurePaymentRails(address token, uint256 minBal) internal {
        bytes memory params = abi.encode(address(buyToken));
        vm.prank(owner);
        paymentRails.configureToken(token, "UNISWAP_SWAP", address(module), minBal, params, true);
    }

    function _configurePaymentRailsFor(PaymentRails pr, address token, uint256 minBal) internal {
        bytes memory params = abi.encode(address(buyToken));
        vm.prank(owner);
        pr.configureToken(token, "UNISWAP_SWAP", address(module), minBal, params, true);
    }
