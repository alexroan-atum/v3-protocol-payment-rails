...
contract BridgeModule is IBridgeModule {
    ...
    function execute(
        ...
        BridgeParams memory bridgeParams = decodeParams(params);
        ...
        if (bridgeParams.healthOracle != address(0)) {
            BridgeHealth memory health = checkBridgeHealth(
    ...
    /// @inheritdoc IBridgeModule
    function decodeParams(bytes calldata encoded) public pure returns (BridgeParams memory params) {
        (
            params.destinationChainId,
            params.destinationAddress,
            params.bridgeAdapter,
    ...
    ) public view returns (BridgeHealth memory health) {
