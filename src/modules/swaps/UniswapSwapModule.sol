...
import { IUniswapSwapModule } from "../../interfaces/IUniswapSwapModule.sol";
...
/// @notice Synchronous swap module that executes atomic swaps through whitelisted Uniswap routers.
/// @dev See {IUniswapSwapModule} for the full architecture, security model, and execution flow.
///
...
contract UniswapSwapModule is IUniswapSwapModule, ActionModuleBase, Ownable2Step {
    ...
    /// @inheritdoc IUniswapSwapModule
    function addRouter(address router) external onlyOwner {
        if (router == address(0)) revert Errors.UniswapSwapModule_ZeroRouter();
        if (router.code.length == 0) revert Errors.UniswapSwapModule_RouterNotContract(router);
        if (_allowedRouters[router]) revert Errors.UniswapSwapModule_RouterAlreadyAdded(router);
    ...
    function removeRouter(address router) external onlyOwner {
        if (!_allowedRouters[router]) revert Errors.UniswapSwapModule_RouterNotAllowed(router);
        ...
        returns (DataTypes.ExecutionResult memory)
    {
        ...
        DataTypes.UniswapSwapParams memory cfg = decodeParams(params);
        DataTypes.UniswapSwapExecutionData memory exec = decodeExecutionData(executionData);
        ...
        if (amountOut < exec.minAmountOut) {
            revert Errors.UniswapSwapModule_InsufficientOutput(amountOut, exec.minAmountOut);
        }
    ...
    /// @inheritdoc IUniswapSwapModule
    function decodeParams(bytes calldata encoded) public pure returns (DataTypes.UniswapSwapParams memory params) {
    ...
    function _validateInputs(
        address token,
        uint256 amount,
        DataTypes.UniswapSwapParams memory cfg,
        DataTypes.UniswapSwapExecutionData memory exec
    )
        private
        view
